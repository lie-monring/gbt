    ...
    <>
      ...
      <div className="fixed inset-y-0 right-0 w-96 bg-[#1a1d27] border-l border-white/5 shadow-2xl z-30 animate-slide-in-right flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {loading ? (
            <p className="text-sm text-[#8b8fa3] text-center mt-8">Loading...</p>
          ) : commit ? (
            <div className="space-y-4">
              <h4 className="text-base font-medium">{commit.subject}</h4>
              <div className="space-y-1.5">
                <div className="flex justify-between text-xs"><span className="text-[#8b8fa3]">Author</span><span>{commit.author}</span></div>
                <div className="flex justify-between text-xs"><span className="text-[#8b8fa3]">Date</span><span>{commit.date?.slice(0, 16).replace('T', ' ')}</span></div>
                <div className="flex justify-between text-xs"><span className="text-[#8b8fa3]">Hash</span><code className="text-[#6c8cf5] font-mono text-xs">{commit.shortHash}</code></div>
                {commit.parents.length > 0 && (
                  <div className="flex justify-between text-xs"><span className="text-[#8b8fa3]">Parents</span><span>{commit.parents.map(p => p.slice(0, 7)).join(', ')}</span></div>
                )}
              </div>
              ...
              <div className="flex flex-col gap-2 pt-3 border-t border-white/5">
                <button onClick={async () => {
                  if (!repoPath) return
                  if (!confirm(`Go back to commit ${commit.shortHash}?\nThis will detach HEAD.`)) return
                ...
                <button onClick={() => navigator.clipboard.writeText(commit.hash)}
                  className="w-full py-1.5 text-xs text-[#8b8fa3] hover:text-white hover:bg-white/5 rounded transition-colors">Copy full hash</button>
              </div>
              {diff && (
                <div className="pt-3 border-t border-white/5">
                  <h5 className="text-xs font-medium text-[#8b8fa3] mb-2">Changed Files</h5>
                  <pre className="text-[11px] font-mono text-[#8b8fa3] whitespace-pre-wrap leading-relaxed">{diff}</pre>
                </div>
              )}
            </div>
          ) : (
            <p className="text-sm text-[#8b8fa3] text-center mt-8">Select a commit to see details</p>
          )}
        </div>
      </div>
    </>
